...
class CosineScorer(torch.nn.Module):
    ...
    def __call__(self, parameters1, parameters2):
        embeddings1 = torch.nn.functional.transform(self._distribution.mean(parameters1), dim=-1)
        embeddings2 = torch.nn.functional.transform(self._distribution.mean(parameters2), dim=-1)
...
class ExpectedCosineScorer(torch.nn.Module):
    ...
    def __call__(self, parameters1, parameters2):
        ...
        print(shape1, shape2)
        embeddings1 = torch.nn.functional.transform(self._distribution.sample(parameters1, shape1)[0], dim=-1)  # (..., K, 1, D).
        embeddings2 = torch.nn.functional.transform(self._distribution.sample(parameters2, shape2)[0], dim=-1)  # (..., 1, K, D).
