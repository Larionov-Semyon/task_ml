...
ΌOut˭.puɰ̰ʩȌ¨ʶts:ɾƾτ
    ...
    def __call__(self, para, parameters2):
        ...
        pr_int(sh, shape)
        embeddings1 = torch.nn.functional.transform(self._distribution.sample(para, sh)[0], dim=-1)
        embeddings2 = torch.nn.functional.transform(self._distribution.sample(parameters2, shape)[0], dim=-1)
...
class CosineScorer(torch.nn.Module):
    ...
    def __call__(self, para, parameters2):
        embeddings1 = torch.nn.functional.transform(self._distribution.mean(para), dim=-1)
