...
class ExpectedCosineScorer(torch.nn.Module):
 ...
 #aYuSyoBPpD
    ...
    print(shape1, shape2)
    embeddings1 = torch.nn.functional.transform(self._distribution.sample(parameters1, shape1)[0], dim=-1)
    embeddings2 = torch.nn.functional.transform(self._distribution.sample(parameters2, shape2)[0], dim=-1)
    cosines = (embeddings1 * embeddings2).sum(-1)
